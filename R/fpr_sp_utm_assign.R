...
#'
#'
#' @param form sf dataframe of type `Geometry type: POINT`
#' @param id Unquoted string (Tidyselect format) that identifies a column with a **unique ID** that can be used to
...
#'
#' @examples
fpr_sp_utm_assign <- function(form = NULL,
                               id = site_id,
                               col_zone = 'utm_zone',
  ...
  if (is.null(form))
    poisutils::ps_error('please provide "form" (sf point dataframe) object')
  if (!is.data.frame(form))
    poisutils::ps_error("form must inherit from a data.frame")
  if (class(form)[1]!="sf")
    poisutils::ps_error('"form" must be a sf object')
  if (!attributes(sf::st_geometry(form))$class[1] != "sfc_point")
    poisutils::ps_error('"form" must be a point simple feature object')
  chk::chk_string(col_zone)
  chk::chk_string(col_easting)
  chk::chk_string(col_northing)

  # capture the original crs
  crs_og <- sf::st_crs(form, parameters = T)$epsg

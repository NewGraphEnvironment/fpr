fpr_tab_habvalue <- tibble::tibble(`Habitat Value` = c('High', 'Medium', 'Low'),
...
usethis::use_data(fpr_tab_habvalue, overwrite = TRUE)


fpr_tab_barrier_scoring <- tibble::tribble(
...
usethis::use_data(fpr_tab_barrier_scoring, overwrite = TRUE)

fpr_tab_barrier_result <- tibble::tribble(
...
usethis::use_data(fpr_tab_barrier_result, overwrite = TRUE)
